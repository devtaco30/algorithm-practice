 ...
 */
class FindDeliveryBox {
    public static void main(String[] args) {
        Solution solution = new Solution();
     
        System.out.println("정답 = 3 / 현재 풀이 값 = " + solution.findDeliveryBox(22,6,8));
        System.out.println("정답 = 4 / 현재 풀이 값 = " + solution.findDeliveryBox(13,3,6));
    }
}
...
class Solution {
    /**
     * 
     * @param n 창고에 있는 택배 상자의 총 개수
     * @param w 가로로 놓는 상자의 개수
     * @param num 꺼내려는 택배 상자의 번호
     * @return
     */
    public int findDeliveryBox(int n, int w, int num) {
        int cnt = 0;
        int currentNum = num;
        
        while (currentNum <= n) {
            // 현재 상자의 열 위치 계산
            int col = (currentNum - 1) % w;
            // 같은 열에서 위로 올라갈 때의 차이
            int diff = (w - col - 1) * 2 + 1;
            currentNum += diff;
            cnt++;
        }
